...
**Puzzle Lab — shipped and live** (July 2026). The math/coding idea from Part 4 returned in a form that fits: not parallel-curriculum worlds on the map, but a self-contained **side-building beside the Trainer School** (the proven Trainer School pattern — building → picker → mode → the standard catch ceremony). No keyboard is needed to play — kids snap block-code together to guide a Pokemon through grid puzzles — and the only typing is the celebratory type-its-name catch that ties it back to the game. Two wings (a five-chapter **Coding** pack: walk/turn/collect → loops → ifs → if/else → AND/OR/NOT, and a four-chapter **Math** pack: counting, times-tables-as-loops, number-line hops, comparisons), **24 puzzle-exclusive Pokemon** appended to the existing worlds (dex total 96 → 120), and four trophies (🧩 Junior Coder, 💻 Master Coder, 🔢 Number Wizard, 🌟 Puzzle Perfect). It honours the design laws below: no clocks or decay, hints teach instead of ever saying "wrong", mastery gates (stars) not time gates, and reduced-motion is respected. Built as its own module (`js/puzzle.js`) with an additive save schema and a ⏭ Step / speed / hint-ladder debug UX for the youngest coders.

**Wave F — the flying isles** (July 2026). The two wings became two **islands the trainer flies to**: the Lab building is now a flight perch (tap → "Where to, trainer?" card → a short skippable bird-flight, instant under reduced-motion), and each pack is a painted islet with its stages laid out as a winding trail of nodes and existing Lost Legends art as chapter landmarks. The old `mathWingOpen` cross-pack gate is gone — **Counting Isle is reachable from the first flight with zero coding progress** (chapter gating still holds within each isle). Save data, trophies, and the catch ceremony are unchanged.
